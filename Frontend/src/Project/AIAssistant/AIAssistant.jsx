...
export default function AIAssistantPage({ setPage }) {
  ...
  async function ask(e){e.preventDefault();if(!message.trim())return;setBusy(true);setError("");try{const r=await apiFetch("/ai/chat/",{method:"POST",body:JSON.stringify({message})});const d=await r.json();setAnswer(d.answer);setMode(d.mode);}catch(e){setError(e.message)}finally{setBusy(false)}}
